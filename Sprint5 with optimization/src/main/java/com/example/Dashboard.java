...
public class Dashboard extends VerticalLayout implements View, Broadcaster.BroadcastListener {
    ...
    VerticalLayout staticLayout1 = new VerticalLayout();
    VerticalLayout staticLayout2 = new VerticalLayout();
    ...
    public Dashboard(){
        ...
        staticLayout1.setHeightUndefined();
        staticLayout1.setHeightUndefined();
        staticLayout2.setHeightUndefined();
        staticLayout1.addComponents(upperSection , profileLayout, pageButtons, liveSessionInfo);
        staticLayout2.addComponents( AllFilterStuff, contentLayout1);
        ...
        sample.setFirstComponent(staticLayout1);
        sample.setSecondComponent(staticLayout2);
        addComponent(sample);
        setSizeFull();
        /**
       ...
       // addComponent(AllFilterStuff);
        ...
        update.addStyleName("primary");
        update.setIcon(VaadinIcons.REFRESH);
        startSession.addStyleName("primary");
        startSession.setIcon(VaadinIcons.MOVIE);
        nextStudent.addStyleName("friendly");
        nextStudent.setIcon(VaadinIcons.ANGLE_DOUBLE_DOWN);
        prevStudent.addStyleName("friendly");
        prevStudent.setIcon(VaadinIcons.ANGLE_DOUBLE_UP);

        liveSessionInfo.addComponents(sessionInfo, studentInfo, update, startSession, prevStudent, nextStudent);
            ...
            public void buttonClick(Button.ClickEvent clickEvent) {
                if(liveOrNot == true){
                    if( currIndexstudent < ListofStudentLists.get(currPage - 1).size()-1 ){
                        currStudent = Integer.toString(ListofStudentLists.get(currPage - 1).get(currIndexstudent +1).getUniqueID());
        ...
        startSession.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent clickEvent) {
                if (liveOrNot == false) {
                    ...
                    currStudent = Integer.toString(ListofStudentLists.get(currPage - 1).get(0).getUniqueID());
                    currIndexstudent = 0;
                    studentInfo.setValue("Current Student: " + currStudent + "   Current Page: " + currLivePage);
                    Broadcaster.broadcast("!cs "+ currStudent + " !cp "+ currLivePage + " !ci " + currIndexstudent);
        ...
        goToPage.addClickListener(e->{
            ...
            if (pageNumbers.getValue() != null) {
                ...
                if (user.currUser.Role.equals("special user")) {
                    ...
                    currStudent = Integer.toString(ListofStudentLists.get(currPage - 1).get(currIndexstudent).getUniqueID());
                    currLivePage = currPage;
                    studentInfo.setValue("Current Student: " + currStudent + "   Current Page: " + currLivePage);
                    Broadcaster.broadcast("!cs " + currStudent + " !cp " + currLivePage + " !ci " + currIndexstudent);
                }
